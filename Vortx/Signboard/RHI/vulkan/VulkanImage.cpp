...
VulkanImage::VulkanImage(VkImage image, ImageFormat format, ImageLayout layout)
	: device(device), format(format), layout(layout)
{
	createImageView();
}
...
void VulkanImage::allocateMemory() {
	...
	allocInfo.memoryTypeIndex = device.findMemoryType(memReq.memoryTypeBits, MemoryProperty::DeviceLocal);
